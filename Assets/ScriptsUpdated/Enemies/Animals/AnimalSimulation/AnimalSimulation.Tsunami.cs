...
{
    public bool TryApplyTsunamiThreatToGroup(
        int groupId,
        float fleeChance01,
        bool instantKillIfFleeFails,
        int damageIfFleeFails,
        int fleeSearchDistance,
        Func<TileCoord, bool> isDangerousTsunamiTile,
        ...
        bool debugLogging)
    {
            ...
            !fleeTile.Equals(originalTile))
        {
            ...
            {
                Debug.Log(
                    $"[AnimalSimulation] Tsunami forced animal group {groupId} to flee " +
        ...
        if (instantKillIfFleeFails)
        {
            ...
            {
                Debug.Log(
                    $"[AnimalSimulation] Tsunami killed animal group {groupId} at {originalTile}. " +
                    $"Flee failed."
                );
            }
        ...
        if (group.currentHealth <= 0 || group.size <= 0)
        {
            ...
            {
                Debug.Log(
                    $"[AnimalSimulation] Tsunami damage killed animal group {groupId} at {originalTile}. " +
                    $"Damage={finalDamage}"
                );
            }
        ...
        {
            Debug.Log(
                $"[AnimalSimulation] Tsunami damaged animal group {groupId} at {originalTile}. " +
