...
{
    public bool TryApplyFloodThreatToGroup(
        int groupId,
        float fleeChance01,
        bool instantKillIfFleeFails,
        int damageIfFleeFails,
        int fleeSearchDistance,
        Func<TileCoord, bool> isDangerousFloodTile,
        ...
        bool debugLogging)
    {
            ...
            !fleeTile.Equals(originalTile))
        {
            ...
            {
                Debug.Log(
                    $"[AnimalSimulation] Flood forced animal group {groupId} to flee " +
        ...
        if (instantKillIfFleeFails)
        {
            ...
            {
                Debug.Log(
                    $"[AnimalSimulation] Flood killed animal group {groupId} at {originalTile}. " +
                    $"Flee failed."
                );
            }
        ...
        if (group.currentHealth <= 0 || group.size <= 0)
        {
            ...
            {
                Debug.Log(
                    $"[AnimalSimulation] Flood damage killed animal group {groupId} at {originalTile}. " +
                    $"Damage={finalDamage}"
                );
            }
        ...
        {
            Debug.Log(
                $"[AnimalSimulation] Flood damaged animal group {groupId} at {originalTile}. " +
