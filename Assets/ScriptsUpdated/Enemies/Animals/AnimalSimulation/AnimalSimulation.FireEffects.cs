...
{
    public bool TryApplyFireThreatToGroup(
        int groupId,
        float fleeChance01,
        bool instantKillIfFleeFails,
        int damageIfFleeFails,
        int fleeSearchDistance,
        Func<TileCoord, bool> isFireOrIncomingFireAtTile,
        ...
        bool debugLogging)
    {
            ...
            !fleeTile.Equals(originalTile))
        {
            ...
            {
                //Debug.Log(
                    //$"[AnimalSimulation] Fire forced animal group {groupId} to flee " +
        ...
        if (instantKillIfFleeFails)
        {
            ...
            {
                //Debug.Log(
                    //$"[AnimalSimulation] Fire killed animal group {groupId} at {originalTile}. " +
                    //$"Flee failed.");
            }
        ...
        if (group.currentHealth <= 0 || group.size <= 0)
        {
            ...
            {
                //Debug.Log(
                    //$"[AnimalSimulation] Fire damage killed animal group {groupId} at {originalTile}. " +
                    //$"Damage={finalDamage}");
            }
        ...
        {
            //Debug.Log(
                //$"[AnimalSimulation] Fire damaged animal group {groupId} at {originalTile}. " +
    ...
    private bool TryFindFireFleeTile(
        ...
        out TileCoord bestTile)
    {
        ...
        for (int i = 0; i < neighbours.Count; i++)
        {
            ...
            if (isFireOrIncomingFireAtTile != null && isFireOrIncomingFireAtTile(coord))
                continue;
