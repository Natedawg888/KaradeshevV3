...
public class ClimateManager : MonoBehaviour
{
    ...
    private PlanetaryForcingSample EvaluatePlanetaryForcingSample()
    {
        ...
        // Gravitational perturbations modulate the raw cycle signals before they drive the sample.
    ...
    // and shifts the precession phase through apsidal perturbation.
    private void ApplyJupiterGravitationalInfluence(
        ref float ecc, ref float obl, ref float pre,
        PlanetaryForcingSettings settings, float t01, float twoPi)
    {
        JupiterGravitationalInfluence j = settings.jupiterGravity;
        if (!j.enabled || j.strength <= 0f) return;

        float s = j.strength;
        float eccMod = Mathf.Sin(t01 * twoPi * j.eccModFrequency);
        ecc *= 1f + eccMod * j.eccModDepth * s;

        float apsidal = Mathf.Sin(t01 * twoPi * j.apsidalFrequency);
        pre += apsidal * j.apsidalAmplitude * s;
    }

    // Venus drives the long ~400kyr eccentricity envelope (modulating how large
    // the 100kyr eccentricity swings are) and couples weakly into obliquity.
    private void ApplyVenusGravitationalInfluence(
        ref float ecc, ref float obl, ref float pre,
        PlanetaryForcingSettings settings, float t01, float twoPi)
    {
        VenusGravitationalInfluence v = settings.venusGravity;
        if (!v.enabled || v.strength <= 0f) return;

        float s = v.strength;
        float longEnv = Mathf.Sin(t01 * twoPi * v.longEccFrequency);
        ecc *= 1f + longEnv * v.longEccDepth * s;

        float oblCoupling = Mathf.Sin(t01 * twoPi * v.obliquityInfluenceFrequency);
        obl += oblCoupling * v.obliquityInfluenceAmplitude * s;
    }

    // Mars drives fast small eccentricity perturbations from its shorter orbit,
    // a weak obliquity coupling through inclination, and a near-resonance beat.
    private void ApplyMarsGravitationalInfluence(
        ref float ecc, ref float obl, ref float pre,
        PlanetaryForcingSettings settings, float t01, float twoPi)
    {
        MarsGravitationalInfluence m = settings.marsGravity;
        if (!m.enabled || m.strength <= 0f) return;

        float s = m.strength;
        float marsEcc = Mathf.Sin(t01 * twoPi * m.eccPerturbFrequency);
        ecc += marsEcc * m.eccPerturbAmplitude * s;

        float marsObl = Mathf.Sin(t01 * twoPi * m.oblPerturbFrequency);
        obl += marsObl * m.oblPerturbAmplitude * s;
