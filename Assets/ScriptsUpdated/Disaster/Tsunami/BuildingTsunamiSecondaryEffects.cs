...
public class BuildingTsunamiSecondaryEffects : MonoBehaviour
{
    [Header("Core Building Tsunami Damage")]
    [SerializeField] private bool applyCoreBuildingDamage = true;

    [Tooltip("1 = use final tsunami damage as-is. 0.5 = half damage. 2 = double damage.")]
    [Min(0f)]
    ...
    [Tooltip("Used to turn incoming tsunami damage into a 0-1-ish severity ratio.")]
    [Min(1)]
    [SerializeField] private int baseDamageReference = 20;

    [Min(0f)][SerializeField] private float shelterCasualtyMultiplier = 0.25f;
    [Min(0f)][SerializeField] private float craftingImpactMultiplier = 0.35f;
    ...
    [Header("Storage Tsunami Effects")]
    [Tooltip("Scales how much stored resource is destroyed/spoiled from tsunami damage severity.")]
    [Min(0f)]
    [SerializeField] private float storageResourceLossMultiplier = 0.25f;

    [Tooltip("Maximum total resource units destroyed from this storage building per tsunami hit. 0 = no cap.")]
    [Min(0)]
    [SerializeField] private int maxStorageResourcesDestroyedPerHit = 8;

    [Tooltip("If false, spoiled food stacks are ignored by tsunami resource destruction.")]
    [SerializeField] private bool tsunamiCanDestroySpoiledFood = true;
    ...
    public void ApplyTsunamiDamage(TsunamiBuildingHitData hitData)
    {
        EnsureRefs();
    ...
    public void OnTsunamiHit(TsunamiBuildingHitData hitData)
    {
